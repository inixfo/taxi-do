            ...
            foreach ($paymentMethods as $paymentMethod) {
                if ($paymentMethod['slug'] == $payment) {
                    if (Module::has($paymentMethod['name'])) {
                        ...
                        return response()->json(['error' => "Module '{$paymentMethod['name']}' not found. Check if the module directory exists."], 400);
                    }
                }
            }

            return response()->json(['error' => __('static.payment_methods.invalid_msg') . " (slug: {$payment})"], 400);

        } catch (Exception $e) {

            return response()->json([
                'error' => __('static.something_went_wrong') . ': ' . $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine()
            ], 500);
        }
