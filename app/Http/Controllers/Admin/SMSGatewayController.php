            ...
            foreach ($smsGateways as $smsGateway) {
                if ($smsGateway['slug'] == $sms) {
                    if (Module::has($smsGateway['name'])) {
                        ...
                        return response()->json(['error' => "Module '{$smsGateway['name']}' not found. Check if the module directory exists."], 400);
                    }
                }
            }

            return response()->json(['error' => __('static.sms_gateways.invalid_msg') . " (slug: {$sms})"], 400);
